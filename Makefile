...
gtk.gso: gtk.c
	gcc -o gtk.gso -I$(MINIGRACE_HEADERS) `pkg-config --cflags gtk+-$(GTK_VERSION)` `pkg-config --libs gtk+-$(GTK_VERSION)` $(UNICODE_LDFLAGS) -fPIC -shared gtk.c

gdk.c: gwrap.py
	$(PYTHON) gwrap.py $(HEADER_LOCATION)/gdk/gdk.h $(HEADER_LOCATION)/gdk/gdkkeysyms.h > gdk.c

gdk.gso: gdk.c
	gcc -o gdk.gso -I$(MINIGRACE_HEADERS) `pkg-config --cflags gtk+-$(GTK_VERSION)` `pkg-config --libs gtk+-$(GTK_VERSION)` $(UNICODE_LDFLAGS) -fPIC -shared gdk.c

cairo.c: gwrap.py
	$(PYTHON) gwrap.py $(CAIRO_INCLUDE_DIR)/cairo/cairo.h > cairo.c

cairo.gso: cairo.c
	gcc -o cairo.gso -I$(MINIGRACE_HEADERS) `pkg-config --cflags gtk+-$(GTK_VERSION)` `pkg-config --libs gtk+-$(GTK_VERSION)` $(UNICODE_LDFLAGS) -fPIC -shared cairo.c
